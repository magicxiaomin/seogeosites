...
def main():
    ...
    launch_rel = None
    launch_path = run / "outputs" / "launch_readiness_pack.md"
    if launch_path.exists():
        public_launch = public / "launch-readiness-pack.md"
        public_launch.write_text(launch_path.read_text(encoding="utf-8"), encoding="utf-8")
        launch_rel = public_launch.relative_to(public)
    simulation_rel = None
    ...
    strategy_review_rel = None
    strategy_review_path = run / "outputs" / "opportunity_strategy_review.md"
    if strategy_review_path.exists():
